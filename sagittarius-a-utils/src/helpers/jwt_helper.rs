...
//use crate::service::userservice::id_existis;
...
// TODO: This.
// pub async fn validate_token_checking_user_profile(
//     token: &String,
//     user_id: &UserId,
// ) -> Result<bool, error::Unspecified> {
//     if token.is_empty() {
...
pub async fn generate_user_token(usr: &User, expires: bool, duration_in_minutes:i64) -> Result<String, error::Unspecified> {
    let key: Hmac<Sha384> = Hmac::new_from_slice(b"KEY_APP_00001").unwrap();
...
pub async fn validate_token(token: &String) -> Result<bool, error::Unspecified> {
    if token.is_empty() {
        return Ok(false);
    }
